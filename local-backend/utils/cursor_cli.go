...
func (c *CursorProvider) GetProviderName() string {
	return "cursor"
}

// IsConfigured checks if the provider is properly configured
func (c *CursorProvider) IsConfigured() bool {
...
// ExecuteTask executes a task using Cursor CLI
func (c *CursorProvider) ExecuteTask(ctx context.Context, request *AITaskRequest) (*AITaskResponse, error) {
	startTime := time.Now()
	...
	args := c.buildCursorCommand(request)
...
// buildCursorCommand builds the Cursor CLI command arguments
func (c *CursorProvider) buildCursorCommand(request *AITaskRequest) []string {
	args := []string{}

	// Basic cursor command
	...
	if options, ok := request.Options["cursor"].(map[string]interface{}); ok {
...
// executeCommand executes the Cursor CLI command
func (c *CursorProvider) executeCommand(ctx context.Context, args []string, workingDir string) (*CommandResult, error) {
	// Create command
	...
	cmd.Env = append(os.Environ(),
		fmt.Sprintf("CURSOR_API_KEY=%s", c.APIKey),
	)
...
// parseTokensUsed attempts to parse token usage from Cursor CLI output
func (c *CursorProvider) parseTokensUsed(output string) int {
	// Cursor CLI might output token usage in various formats
...
// CreateCursorScript creates a temporary script file for complex operations
func (c *CursorProvider) CreateCursorScript(content string, workingDir string) (string, error) {
	// Create temporary file
	tmpFile, err := os.CreateTemp(workingDir, "cursor-script-*.md")
	if err != nil {
...
func (c *CursorProvider) CleanupScript(scriptPath string) error {
...
// GetCursorVersion returns the version of Cursor CLI
func (c *CursorProvider) GetCursorVersion() (string, error) {
...
// ValidateCursorConfig validates the Cursor configuration
func (c *CursorProvider) ValidateCursorConfig() error {
	if !c.IsConfigured() {
		return fmt.Errorf("Cursor CLI is not properly configured")
	}

	// Test basic functionality
	cmd := exec.Command(c.CLIPath, "--help")
	if err := cmd.Run(); err != nil {
		return fmt.Errorf("Cursor CLI is not working properly: %w", err)
	}
