...
// Mock task data - in a real app this would come from a service
interface TaskStats {
  total: number;
  completed: number;
  inProgress: number;
...
export function DashboardTab({ repository }: DashboardTabProps) {
  ...
  const taskStats = generateMockTaskStats();
  ...
  const githubStats = useMemo(() => {
    ...
    return {
      ...
      pullRequests: {
        ...
        total: prs.length,
        recent: recentPRs,
        mergeRate: prs.length > 0 ? Math.round((mergedPRs / prs.length) * 100) : 0
      }
    };
  }, [issuesData, prsData]);
  ...
  const hasError = issuesError || prsError;
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <MetricCard
          ...
          subtitle={`${githubStats?.issues.total || 0} total`}
          icon={AlertCircle}
          trend="neutral"
          color="text-red-500"
          loading={isLoading}
        ...
        <MetricCard
          ...
          value={`${taskStats.avgCompletionTime}h`}
          subtitle="per task"
          icon={Clock}
          trend="down"
          color="text-blue-500"
              ...
              <>
                ...
                <div>
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <Badge variant="outline">{githubStats.issues.total} total</Badge>
                  </div>
                ...
                <div>
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <Badge variant="outline">{githubStats.pullRequests.total} total</Badge>
                  </div>
