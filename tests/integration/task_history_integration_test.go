...
func TestE2E_HappyPath_QueueToAPIRetrieval(t *testing.T) {
	...
	t.Run("complete workflow: submit task -> queue -> database -> api retrieval", func(t *testing.T) {
		...
		dbRecord.Status = models.WorkflowStatusCompleted
		dbRecord.CompletedAt = timePtr(time.Now())
		dbRecord.ProcessingTimeMs = int64Ptr(30000) // 30 seconds
		dbRecord.Result = stringPtr("Authentication system implemented successfully")
		err = suite.db.Save(&dbRecord).Error
		require.NoError(t, err)
...
func TestAtomicTransactions_SuccessAndFailure(t *testing.T) {
	...
	t.Run("atomic success: both database and queue operations succeed", func(t *testing.T) {
		...
		err := suite.atomicService.CreateWorkflowAndQueue(context.Background(), workflowData)
		assert.NoError(t, err)

		// Verify database record exists
		var dbRecord models.WorkflowHistory
		err = suite.db.Where("request_id = ?", workflowData.RequestID).First(&dbRecord).Error
		assert.NoError(t, err)
	...
	t.Run("atomic failure: queue failure should rollback database", func(t *testing.T) {
		...
		var count int64
		suite.db.Model(&models.WorkflowHistory{}).Where("request_id = ?", workflowData.RequestID).Count(&count)
...
func TestErrorScenarios_DatabaseAndNetwork(t *testing.T) {
	...
	t.Run("database connection error simulation", func(t *testing.T) {
		...
		// Re-setup database for other tests
		suite.db, err = gorm.Open(sqlite.Open(":memory:"), &gorm.Config{})
		require.NoError(t, err)
		err = suite.db.AutoMigrate(&models.WorkflowHistory{})
		require.NoError(t, err)
	})
